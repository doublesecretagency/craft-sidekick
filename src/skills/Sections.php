...
class Sections extends BaseSkillSet
{
    ...
    public static function createSection(string $sectionConfig, string $siteSettingsConfig): SkillResponse
    {
        ...
        // If the section type is not valid, throw an exception
        if (!in_array($sectionType, self::SECTION_TYPES, true)) {
            $types = implode(', ', self::SECTION_TYPES);
            throw new Exception("Invalid section type: {$sectionType}. Valid types are: {$types}");
        }
        ...
        // If the section is not valid, throw an exception
        if (!$section->validate()) {
            $errors = implode(', ', $section->getErrorSummary(true));
            throw new Exception("Invalid section configuration: {$errors}");
        }

        // If unable to save the section, throw an exception
        if (!VersionHelper::sectionsService()->saveSection($section)) {
            $errors = implode(', ', $section->getErrorSummary(true));
            throw new Exception("Unable to create section: {$errors}");
        }
...
//            'response' => $config,
    ...
    public static function updateSection(string $sectionHandle, string $newConfig): SkillResponse
    {
        ...
        if (!$section) {
            throw new Exception("Unable to update, section `{$sectionHandle}` does not exist.");
        }
        ...
        if (!is_array($config)) {
            throw new Exception("Invalid JSON provided for section configuration.");
        }
...
//            $section->propagationKeyFormat = ($config['propagationKeyFormat'] ?? $section->propagationKeyFormat);
        ...
        if (!VersionHelper::sectionsService()->saveSection($section)) {
            $errors = implode(', ', $section->getErrorSummary(true));
            throw new Exception("Unable to update section: {$errors}");
        }
...
//            'response' => $config,
    ...
    public static function deleteSection(string $handle): SkillResponse
    {
        ...
        // If the section doesn't exist, throw an exception
        if (!$section) {
            throw new Exception("Section \"{$handle}\" not found.");
        }

        // If unable to delete the section, throw an exception
        if (!$service->deleteSection($section)) {
            $errors = implode(', ', $section->getErrorSummary(true));
            throw new Exception("Unable to delete section: {$errors}");
        }
